...
严格零付费部署可使用 Supabase Free Storage 保存公共版本：免费项目提供 1 GB 存储，超出配额后限制服务而不是自动产生超额账单。Supabase 的 S3 接口要求 path-style 地址，因此需要设置 `PUBLIC_VERSION_S3_FORCE_PATH_STYLE=true`。线上 `light` 模式还应把 `MAX_INDEX_SNAPSHOT_BYTES` 控制在 50 MB 单文件限制以内；当前建议值为 `47185920`（45 MB），并保留 5 MB 安全余量。生产环境还应设置 `REQUIRE_PERSISTENT_VERSION_STORAGE=true`，避免配置错误时静默回退到临时磁盘。独立 Render Background Worker 没有免费实例，所以零付费方案继续使用进程内 Worker。
...
REQUIRE_PERSISTENT_VERSION_STORAGE=false
...
本地开发保持 `PUBLIC_VERSION_STORAGE_BACKEND=local` 即可；`PUBLIC_VERSION_STORAGE_DIR` 为空时使用 `backend/public_versions/`。Render 等临时磁盘环境应改为 `s3`，并设置 `REQUIRE_PERSISTENT_VERSION_STORAGE=true`，让误配在启动阶段直接失败。生产环境还需配置 bucket、前缀、endpoint、region 以及标准 AWS 访问密钥。Supabase Storage 需要设置 `PUBLIC_VERSION_S3_FORCE_PATH_STYLE=true`；AWS S3 等使用默认虚拟主机地址的服务保持 `false`。原始 PDF 以 ZIP 版本包持久化，索引在发布、回滚或启动恢复时按当前 RAG 模式重建，避免把特定向量库格式锁死在版本文件中。
