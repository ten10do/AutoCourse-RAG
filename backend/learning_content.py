...
def generate_hierarchical_learning_content(
    ...
    for index, batch in enumerate(batches, start=1):
        partial_prompt = (
            f"{task_prompt}\n\n"
            f"当前是全部课程资料的第 {index}/{len(batches)} 批。"
        ...
        partials.append(
            (
                SimpleNamespace(
                    ...
                    metadata={"source": f"课程资料批次 {index}", "page": index - 1},
                ),
                0.0,
            )
        )
    ...
    final_prompt = (
        f"{task_prompt}\n\n"
        "下面的参考片段是覆盖全部课程资料后生成的分批提要。"
        "请去重、合并跨章节关联，并生成整门课程的最终结果。"
    )
